        ...
        node('maven') {
            ...
            stage('config') {
                dir('config') {
                    ...
                    sh('oc create secret generic ribn-dev-pi-config-secret --from-file=./configuration/configuration/environment.properties,./configuration/configuration/app/standalone/configuration/sso/sso.keystore')
                }
            }
            stage('deploy') {
                dir('scm') {
